...
const Product = ({
  ...
  const [productFavouritedArray, setProductFavouritedArray] = useState(
    productObj?.userFavourited
  );
  ...
  function checkIfUserAddedToFavourite(userFavourites, id) {
    ...
    if (userFavourites) {
      let arrayItem = data?.favouritesArray?.find(
        ...
        {firebaseUserInfo?.accountVerified &&
          (isFavourited && checkIfUserAddedToFavourite(userFavourites, id) ? (
            <div
              ...
              className={`absolute bg-[#695acde4] top-0 right-0 rounded-t-nne rounded-br-none rounded-bl-xl runded rounded-tr-xl p-[0.3rem]`}
            >
              <Image
                src={favouriteClicked}
                width={20}
                height={20}
                className={`scaleLikeIcon`}
                alt="favourited"
              />
            </div>
          ) : (
